...
int main() {
    ...
    if(result < 0) {
        ...
        if((my_fifo_fd = open(my_fifo_name, O_WRONLY)) < 0) {
            printf("rw01-parent: Can\'t open my FIFO for writting\n");
            exit(-1);
        }
        size = write(my_fifo_fd, my_msg, msg_size);
        if(size != msg_size) {
            printf("rw01-parent: Can\'t write all my string to my FIFO\n");
            exit(-1);
        }
        if(close(my_fifo_fd) < 0) {
            printf("rw01-parent: Can\'t close my FIFO\n");
            exit(-1);
        }
        printf("rw01-parent exit\n");
    } else { // Потомок. Читает из чужого канала
        if((ext_fifo_fd = open(ext_fifo_name, O_RDONLY)) < 0){
            printf("rw01-child: Can\'t open external FIFO for reading\n");
            exit(-1);
        }
        size = read(ext_fifo_fd, str_buf, buf_size);
        if(size < 0) {
            printf("rw01-child: Can\'t read external string from FIFO\n");
            exit(-1);
        }
        printf("rw01-child: exit, str_buf: %s\n", str_buf);
        if(close(ext_fifo_fd) < 0){
            printf("rw01-child: Can\'t close external FIFO\n");
            exit(-1);
        }
        printf("rw01-child exit\n");
    }
    return 0;
}
