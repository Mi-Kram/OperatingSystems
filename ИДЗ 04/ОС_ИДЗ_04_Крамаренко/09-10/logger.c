...
int main(int argc, char* argv[]) {
  ...
  while (running) {
    // wait for messages
    if (recvfrom(sock, &msg, sizeof(msg), 0, (struct sockaddr*)&server_addr, &server_len) <= 0) break;
    if (!running || msg.type_id == EXIT_TYPE) break;

    // Log messages based on their source
    switch (msg.from_id) {
      case IVANOV_ID:
        if (msg.type_id == COMMON_TYPE) printf("Ivanov stole \"%s\"\n", msg.message);
        else if (msg.type_id == DONE_TYPE) printf("Ivanov stole everything\n");
        break;
