...
int main(int argc, char* argv[]) {
  ...
  Message reg_msg = {PETROV_ID, REGISTER_TYPE};
  ...
  while (running) {
    ...
    // Process and report item status
    income_msg.from_id = PETROV_ID;
    printf("Petrov uploaded \"%s\"\n", income_msg.message);
    if (sendto(sock, &income_msg, sizeof(income_msg), 0, (struct sockaddr*)&server_addr, server_len) <= 0) break;
  }
