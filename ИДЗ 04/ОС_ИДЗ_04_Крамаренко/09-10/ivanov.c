...
int main(int argc, char* argv[]) {
  ...
  // Set number of items to steal
  n = argc == 4 ? atoi(argv[3]) : 10;
  srand(time(NULL));

  // Register with the server
  Message reg_msg = {IVANOV_ID, REGISTER_TYPE};
  ...
  while (running) {
    ...
    // generate message.
    Message msg = {IVANOV_ID, COMMON_TYPE};
  ...
  // Send completion message if all items stolen
  if (n < cur) {
    printf("Ivanov stole everything\n");
    Message final_msg = {IVANOV_ID, DONE_TYPE};
    sendto(sock, &final_msg, sizeof(final_msg), 0, (struct sockaddr*)&server_addr, server_len);
  }
