...
void sigfunc(int sig) {
  ...
  if (sig == SIGINT) {
    ...
    for (int i = 0; i < 2; ++i) {
      if (buffer->reader_pids[i] > 0) {
        kill(buffer->reader_pids[i], SIGTERM);
        printf("  --> SIGTERM sent to Reader %d\n", buffer->reader_pids[i]);
      }
    }
...
int main() {
  ...
  while (1) {
    ...
    if (sem_wait(writer_mutex) == -1) {
      perror("sem_wait: Incorrect wait of mutex");
      exit(-1);
    };
    // Текущая свободная ячейка для записи. Она должна быть. Иначе сюда не попасть
    int i = buffer->head;
    // Запись в первую свободную ячейку
    buffer->store[i] = rand() % 11;  // число от 0 до 10
    // Переместить индекс на слежующую ячейку.
