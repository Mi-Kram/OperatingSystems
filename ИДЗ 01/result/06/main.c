...
/// @param output_fd Descriptor of a pipe to write the file content.
/// @return Returns -1 if something went wrong.
...
/// @param input_fd Descriptor of a pipe to read from.
/// @param path File path to write to.
/// @return Returns -1 if something went wrong.
int writeData(int input_fd, char* path);
...
int main(int argc, char** argv) {
  ...
  // Read file.
  if (readFile(argv[1], inputPipe[1]) == -1) {
  ...
  // Write data.
  if (writeData(outputPipe[0], argv[2]) == -1) {
