...
// Name of shared memory and semaphores.
#define SHM_NAME "/military_object"
#define SEM_IVANOV "/sem_ivanov"
#define SEM_PETROV "/sem_petrov"
#define SEM_NECHEPORUK "/sem_necheporuk"
...
int main(int argc, char** argv) {
  ...
  // start processes.
  if (start_ivanov_process(sem_ivanov, sem_petrov, shared) == -1) {
    dispose();
    return 0;
  }
  if (start_petrov_process(sem_petrov, sem_necheporuk, shared) == -1) {
    dispose();
    return 0;
  }
  if (start_necheporuk_process(sem_necheporuk, sem_ivanov, shared) == -1) {
    dispose();
    return 0;
  }
